...
//		$pv				=  intval($this->db->queryFirstField("SELECT sum(amount) FROM personal_volume WHERE member_id = %d AND date > NOW() - INTERVAL $trace_pv_days day", $memberId));
        ...
        $res = $this->db->query("SELECT member_id FROM members WHERE sponsor_id = %d AND membership != '0'", $memberId);

        $arPaid_referrals = [];
        foreach ($res as $v) {
            $arPaid_referrals[] = $v['member_id'];
        }
        $paid_referrals = count($arPaid_referrals);

        if ($paid_referrals) {
            $pv = intval($this->db->queryFirstField("
			SELECT sum(amount) 
			FROM wallet_deposit 
			WHERE member_id IN (" . implode(',', $arPaid_referrals) . ") 
				AND amount > 0 
				AND transaction_date > UNIX_TIMESTAMP(NOW() - INTERVAL $trace_pv_days day)", $memberId));
        } else {
            $pv = 0;
        }
