...
$data_first = $db->search('bsc_products')->
        setColumns('*')->
        addFilter("visible=%d", 1)->
        addFilter("idCategory<>%d", 7)->
        sortByOrdering('ASC')->
        setRange(EW_PAGINATION)->
        setPage($currentpage);
if($category > 0) $data_first = $data_first->
        addFilter("idCategory=%d", $category);

$rows = $data_first->getRows();		
    ...
    ?>" data-quantity="1">
        <div class="thumbnail">
            ...
            <div class="caption">
                ...
                <p align="center">
                   <?php /* <a href="product_sheet.php?id=<?php echo $rs['id']; ?>" class="btn btn-default"><i class="icon-plus"></i>More</a> */?>
                   	<a href="/system/app/plugins/cart/product_sheet_inside.php?id=<?php echo $rs['id']; ?>" class="btn btn-default" style="font-size: smaller;"><i class="icon-plus"></i>More<font color="green">/M&#225;s</font></a>
                    <?php if ($shoppingcart_vertical) { ?>
                        <a href="#" class="btn btn-success addproduct" style="font-size: smaller;"><i class="icon-shopping-cart"></i>Add<font color="green">/Agregar</font></a>
                    <?php } else { ?>
                        <a href="#" class="btn btn-success addproduct-horizontal" style="font-size: smaller;"><i class="icon-shopping-cart"></i>Add<font color="green">/Agregar</font></a>
                    <?php } ?>
                </p>
            </div>
        </div>
