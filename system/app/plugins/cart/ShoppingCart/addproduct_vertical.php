...
<div class="ShoppingCartHead" style="width: 165px;font-size: 10px;">
    <img  id="BSC_animation_cart" src="images/shoppingcart.png" />Shopping<font color="green">/Comprando</font>
            ...
            if (!$cart->get_cart()) {
                ...
                echo "<div class='ShoppingCartFoot'>";
                echo "<div id='BSC_animation_count' >Total " . $total_products . " Service(s)<br/>";
                echo moneyformat($total_price);
                if (EW_SHIPPING > 0) {
                    echo " + shipping " . moneyformat(EW_SHIPPING);
                } //shipping? 
                echo "</div><br/><br/>";
                echo "<a href='/store/".$_SESSION['enroller']."/cart/'>View shopping cart / Pay
                    <hr style='margin: 8px;''>
                    Ver carrito de compras / Pagar
                </a>";
                echo "</div>";
            }
            /*
...
<script>
  ...
  //#delete_button
    $(".delete").click(function(e) {
        ...
        $("#BSCart").load("ShoppingCart/addproduct_vertical.php?action=delete&id=" + id + "&type="+type);
        e.preventDefault();
    });
</script>