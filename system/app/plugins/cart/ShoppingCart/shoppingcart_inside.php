<form action="ShoppingCart/shoppingcart_update_inside.php" method="post">

    <table class="ShoppingCartHead" width="100%" height="40" border="0" cellspacing="0" cellpadding="0" >
        <tr>
            <td width="12%" align="left">&nbsp;&nbsp;Image</td>
            <td width="30%" align="left">Nombre</td>
            <td width="16%" align="center">Precio</td>
            <td width="2%"></td>
    ...
    foreach ($shoppingcart as $cart) {
        ...
        <table class="table-striped" width="100%" height="80" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td width="9%" align="center"><a href="ShoppingCart/shoppingcart_delete_inside.php?id=<?php echo $cart['id_product']; ?>&type=<?php
                    if ($cart['id_type'] == 0) {
...
// are shipping ?? configure shipping into config.php
    ...
    if (count($row) > 0) {
        ...
        <div style="margin-top: 30px; margin-bottom: 30px; text-align: center; background-color: #f5f5f5;" class="ShoppingCartFoot" > 
            <label class="control-label">¿Tienes un cupón de descuento? Entrar Codigo</label>
            <input name="coupon" id="coupon_input" />
        </div>
        <?php
    }
	else {
		echo '<input type="hidden" name="coupon" id="coupon_input" />';
	}
    ?>

    <!-- Buttons -->
    <div class="ShoppingCartButtons">
        <a class="btn btn-default" href="<?php echo $_SESSION['LastProductPage']; ?>">Ir a Servicios</a>                            
        <button class="btn" type="submit">Actualizar Cantidades</button>
        <a class="btn btn-success checkout_button" >Pagar</a><?php //call js click id   ?>
