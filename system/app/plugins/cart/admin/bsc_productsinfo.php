...
class cbsc_products extends cTable {
	...
	function __construct() {
		...
		// shipping
		$this->shipping = new cField('bsc_products', 'bsc_products', 'x_shipping', 'shipping', '`shipping`', '`shipping`', 201, -1, FALSE, '`shipping`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->shipping->Exportable) $Doc->ExportCaption($this->shipping);
