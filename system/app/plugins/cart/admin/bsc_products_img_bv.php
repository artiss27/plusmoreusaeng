...
$bsc_products_img_blobview = NULL; // Initialize page object first

class cbsc_products_img_blobview extends cbsc_products {
	...
	// Page object name
	var $PageObjName = 'bsc_products_img_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('bsc_products', 'x_img');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('img');
				...
				if (trim(strval($this->Recordset->fields('img'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('img') . "\"");
				}
...
// Create page object
if (!isset($bsc_products_img_blobview)) $bsc_products_img_blobview = new cbsc_products_img_blobview();

// Page init
$bsc_products_img_blobview->Page_Init();

// Page main
$bsc_products_img_blobview->Page_Main();
?>
<?php
$bsc_products_img_blobview->Page_Terminate();
?>
