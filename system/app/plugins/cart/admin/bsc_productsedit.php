...
class cbsc_products_edit extends cbsc_products {
	...
	function LoadFormValues() {
		...
		if (!$this->shipping->FldIsDetailKey) {
			$this->shipping->setFormValue($objForm->GetValue("x_shipping"));
		}
	...
	function LoadRowValues(&$rs) {
		...
		$this->shipping->setDbValue($rs->fields('shipping'));
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->shipping->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->shipping->FldCaption()));
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// shipping
			$this->shipping->SetDbValueDef($rsnew, $this->shipping->CurrentValue, NULL, $this->shipping->ReadOnly);
...
<tr id="r_shipping"<?php echo $bsc_products->RowAttributes() ?>>
		<td><span id="elh_bsc_products_shipping">Shipping Fees</span></td>
		<td<?php echo $bsc_products->shipping->CellAttributes() ?>><span id="el_bsc_products_shipping" class="control-group">
<textarea data-field="x_shipping" name="x_shipping" id="x_shipping" cols="100" rows="8"<?php echo $bsc_products->shipping->EditAttributes() ?>><?php echo $bsc_products->shipping->EditValue ?></textarea>
</span>"Country Code","shipping_fee" (one per line)<br />"Other","shipping_fee" (For countries not listed on the csv)</td>
	</tr>
