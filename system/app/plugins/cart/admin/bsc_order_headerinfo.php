...
class cbsc_order_header extends cTable {
	...
	function __construct() {
		...
		// shipping_status
		$this->shipping_status = new cField('bsc_order_header', 'bsc_order_header', 'x_shipping_status', 'shipping_status', '`shipping_status`', '`shipping_status`', 200, -1, FALSE, '`shipping_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['shipping_status'] = &$this->shipping_status;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->shipping_status->Exportable) $Doc->ExportCaption($this->shipping_status);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->shipping_status->Exportable) $Doc->ExportField($this->shipping_status);
