...
$bsc_products_download_blobview = NULL; // Initialize page object first

class cbsc_products_download_blobview extends cbsc_products {
	...
	// Page object name
	var $PageObjName = 'bsc_products_download_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('bsc_products', 'x_download');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('download');
				...
				if (trim(strval($this->Recordset->fields('download'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('download') . "\"");
				}
...
// Create page object
if (!isset($bsc_products_download_blobview)) $bsc_products_download_blobview = new cbsc_products_download_blobview();

// Page init
$bsc_products_download_blobview->Page_Init();

// Page main
$bsc_products_download_blobview->Page_Main();
?>
<?php
$bsc_products_download_blobview->Page_Terminate();
?>
