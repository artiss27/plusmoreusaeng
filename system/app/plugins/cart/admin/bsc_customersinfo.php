...
//
// Table class for bsc_customers
//
class cbsc_customers extends cTable {
	var $id;
	var $datelastorder;
	var $dateregister;
	var $payer_email;
	...
	function __construct() {
		...
		$this->TableVar = 'bsc_customers';
		$this->TableName = 'bsc_customers';
		...
		// id
		$this->id = new cField('bsc_customers', 'bsc_customers', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// datelastorder
		$this->datelastorder = new cField('bsc_customers', 'bsc_customers', 'x_datelastorder', 'datelastorder', '`datelastorder`', 'DATE_FORMAT(`datelastorder`, \'%Y/%m/%d\')', 135, 5, FALSE, '`datelastorder`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->datelastorder->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['datelastorder'] = &$this->datelastorder;

		// dateregister
		$this->dateregister = new cField('bsc_customers', 'bsc_customers', 'x_dateregister', 'dateregister', '`dateregister`', 'DATE_FORMAT(`dateregister`, \'%Y/%m/%d\')', 135, 5, FALSE, '`dateregister`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->dateregister->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['dateregister'] = &$this->dateregister;

		// payer_email
		$this->payer_email = new cField('bsc_customers', 'bsc_customers', 'x_payer_email', 'payer_email', '`payer_email`', '`payer_email`', 200, -1, FALSE, '`payer_email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['payer_email'] = &$this->payer_email;

		// first_name
		$this->first_name = new cField('bsc_customers', 'bsc_customers', 'x_first_name', 'first_name', '`first_name`', '`first_name`', 200, -1, FALSE, '`first_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['first_name'] = &$this->first_name;

		// last_name
		$this->last_name = new cField('bsc_customers', 'bsc_customers', 'x_last_name', 'last_name', '`last_name`', '`last_name`', 200, -1, FALSE, '`last_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['last_name'] = &$this->last_name;

		// address_name
		$this->address_name = new cField('bsc_customers', 'bsc_customers', 'x_address_name', 'address_name', '`address_name`', '`address_name`', 200, -1, FALSE, '`address_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address_name'] = &$this->address_name;

		// address_street
		$this->address_street = new cField('bsc_customers', 'bsc_customers', 'x_address_street', 'address_street', '`address_street`', '`address_street`', 200, -1, FALSE, '`address_street`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address_street'] = &$this->address_street;

		// address_city
		$this->address_city = new cField('bsc_customers', 'bsc_customers', 'x_address_city', 'address_city', '`address_city`', '`address_city`', 200, -1, FALSE, '`address_city`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address_city'] = &$this->address_city;

		// address_zip
		$this->address_zip = new cField('bsc_customers', 'bsc_customers', 'x_address_zip', 'address_zip', '`address_zip`', '`address_zip`', 200, -1, FALSE, '`address_zip`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address_zip'] = &$this->address_zip;

		// address_state
		$this->address_state = new cField('bsc_customers', 'bsc_customers', 'x_address_state', 'address_state', '`address_state`', '`address_state`', 200, -1, FALSE, '`address_state`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address_state'] = &$this->address_state;

		// address_country
		$this->address_country = new cField('bsc_customers', 'bsc_customers', 'x_address_country', 'address_country', '`address_country`', '`address_country`', 200, -1, FALSE, '`address_country`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address_country'] = &$this->address_country;

		// address_country_code
		$this->address_country_code = new cField('bsc_customers', 'bsc_customers', 'x_address_country_code', 'address_country_code', '`address_country_code`', '`address_country_code`', 200, -1, FALSE, '`address_country_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("bsc_customersview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("bsc_customersview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "bsc_customersadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("bsc_customersedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("bsc_customersadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("bsc_customersdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->datelastorder->Exportable) $Doc->ExportCaption($this->datelastorder);
				if ($this->dateregister->Exportable) $Doc->ExportCaption($this->dateregister);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->datelastorder->Exportable) $Doc->ExportField($this->datelastorder);
					if ($this->dateregister->Exportable) $Doc->ExportField($this->dateregister);
