...
$bsc_customers_add = NULL; // Initialize page object first

class cbsc_customers_add extends cbsc_customers {
	...
	// Page object name
	var $PageObjName = 'bsc_customers_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "bsc_customersview.php")
	...
	function LoadDefaultValues() {
		$this->datelastorder->CurrentValue = NULL;
		$this->datelastorder->OldValue = $this->datelastorder->CurrentValue;
		$this->dateregister->CurrentValue = NULL;
		$this->dateregister->OldValue = $this->dateregister->CurrentValue;
		$this->payer_email->CurrentValue = NULL;
		$this->payer_email->OldValue = $this->payer_email->CurrentValue;
		...
		$this->address_street->OldValue = $this->address_street->CurrentValue;
		$this->address_city->CurrentValue = NULL;
		$this->address_city->OldValue = $this->address_city->CurrentValue;
		$this->address_zip->CurrentValue = NULL;
		$this->address_zip->OldValue = $this->address_zip->CurrentValue;
		$this->address_state->CurrentValue = NULL;
		$this->address_state->OldValue = $this->address_state->CurrentValue;
		$this->address_country->CurrentValue = NULL;
		$this->address_country->OldValue = $this->address_country->CurrentValue;
		$this->address_country_code->CurrentValue = NULL;
		$this->address_country_code->OldValue = $this->address_country_code->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// datelastorder
		$this->datelastorder->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->datelastorder->CurrentValue, 5), NULL, FALSE);

		// dateregister
		$this->dateregister->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->dateregister->CurrentValue, 5), NULL, FALSE);

		// payer_email
		$this->payer_email->SetDbValueDef($rsnew, $this->payer_email->CurrentValue, NULL, FALSE);
		...
		// address_name
		$this->address_name->SetDbValueDef($rsnew, $this->address_name->CurrentValue, NULL, FALSE);

		// address_street
		$this->address_street->SetDbValueDef($rsnew, $this->address_street->CurrentValue, NULL, FALSE);

		// address_city
		$this->address_city->SetDbValueDef($rsnew, $this->address_city->CurrentValue, NULL, FALSE);

		// address_zip
		$this->address_zip->SetDbValueDef($rsnew, $this->address_zip->CurrentValue, NULL, FALSE);

		// address_state
		$this->address_state->SetDbValueDef($rsnew, $this->address_state->CurrentValue, NULL, FALSE);

		// address_country
		$this->address_country->SetDbValueDef($rsnew, $this->address_country->CurrentValue, NULL, FALSE);

		// address_country_code
		$this->address_country_code->SetDbValueDef($rsnew, $this->address_country_code->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($bsc_customers_add)) $bsc_customers_add = new cbsc_customers_add();

// Page init
$bsc_customers_add->Page_Init();

// Page main
$bsc_customers_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$bsc_customers_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var bsc_customers_add = new ew_Page("bsc_customers_add");
bsc_customers_add.PageID = "add"; // Page ID
var EW_PAGE_ID = bsc_customers_add.PageID; // For backward compatibility

// Form object
var fbsc_customersadd = new ew_Form("fbsc_customersadd");

// Validate form
fbsc_customersadd.Validate = function() {
...
fbsc_customersadd.Form_CustomValidate = 
...
fbsc_customersadd.ValidateRequired = true;
<?php } else { ?>
fbsc_customersadd.ValidateRequired = false; 
<?php } ?>
...
<?php $bsc_customers_add->ShowPageHeader(); ?>
<?php
$bsc_customers_add->ShowMessage();
?>
<form name="fbsc_customersadd" id="fbsc_customersadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="bsc_customers">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_bsc_customersadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fbsc_customersadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$bsc_customers_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$bsc_customers_add->Page_Terminate();
?>
