...
$bsc_types_add = NULL; // Initialize page object first

class cbsc_types_add extends cbsc_types {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("bsc_typeslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "bsc_typesview.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "bsc_typeslist.php", $this->TableVar);
...
// Create page object
if (!isset($bsc_types_add)) $bsc_types_add = new cbsc_types_add();

// Page init
$bsc_types_add->Page_Init();

// Page main
$bsc_types_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$bsc_types_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var bsc_types_add = new ew_Page("bsc_types_add");
bsc_types_add.PageID = "add"; // Page ID
var EW_PAGE_ID = bsc_types_add.PageID; // For backward compatibility

// Form object
var fbsc_typesadd = new ew_Form("fbsc_typesadd");

// Validate form
fbsc_typesadd.Validate = function() {
...
fbsc_typesadd.Form_CustomValidate = 
...
fbsc_typesadd.ValidateRequired = true;
<?php } else { ?>
fbsc_typesadd.ValidateRequired = false; 
<?php } ?>
...
<?php $bsc_types_add->ShowPageHeader(); ?>
<?php
$bsc_types_add->ShowMessage();
?>
<form name="fbsc_typesadd" id="fbsc_typesadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="bsc_types">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_bsc_typesadd" class="table table-bordered table-striped">
<?php if ($bsc_types->name->Visible) { // name ?>
	<tr id="r_name"<?php echo $bsc_types->RowAttributes() ?>>
		<td><span id="elh_bsc_types_name"><?php echo $bsc_types->name->FldCaption() ?></span></td>
		<td<?php echo $bsc_types->name->CellAttributes() ?>><span id="el_bsc_types_name" class="control-group">
<input type="text" data-field="x_name" name="x_name" id="x_name" size="200" maxlength="255" placeholder="<?php echo $bsc_types->name->PlaceHolder ?>" value="<?php echo $bsc_types->name->EditValue ?>"<?php echo $bsc_types->name->EditAttributes() ?>>
</span><?php echo $bsc_types->name->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($bsc_types->price->Visible) { // price ?>
	<tr id="r_price"<?php echo $bsc_types->RowAttributes() ?>>
		<td><span id="elh_bsc_types_price"><?php echo $bsc_types->price->FldCaption() ?></span></td>
		<td<?php echo $bsc_types->price->CellAttributes() ?>><span id="el_bsc_types_price" class="control-group">
<input type="text" data-field="x_price" name="x_price" id="x_price" size="30" placeholder="<?php echo $bsc_types->price->PlaceHolder ?>" value="<?php echo $bsc_types->price->EditValue ?>"<?php echo $bsc_types->price->EditAttributes() ?>>
</span><?php echo $bsc_types->price->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($bsc_types->price_offer->Visible) { // price_offer ?>
	<tr id="r_price_offer"<?php echo $bsc_types->RowAttributes() ?>>
		<td><span id="elh_bsc_types_price_offer"><?php echo $bsc_types->price_offer->FldCaption() ?></span></td>
		<td<?php echo $bsc_types->price_offer->CellAttributes() ?>><span id="el_bsc_types_price_offer" class="control-group">
<input type="text" data-field="x_price_offer" name="x_price_offer" id="x_price_offer" size="30" placeholder="<?php echo $bsc_types->price_offer->PlaceHolder ?>" value="<?php echo $bsc_types->price_offer->EditValue ?>"<?php echo $bsc_types->price_offer->EditAttributes() ?>>
</span><?php echo $bsc_types->price_offer->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($bsc_types->ordering->Visible) { // ordering ?>
	<tr id="r_ordering"<?php echo $bsc_types->RowAttributes() ?>>
		<td><span id="elh_bsc_types_ordering"><?php echo $bsc_types->ordering->FldCaption() ?></span></td>
		<td<?php echo $bsc_types->ordering->CellAttributes() ?>><span id="el_bsc_types_ordering" class="control-group">
<input type="text" data-field="x_ordering" name="x_ordering" id="x_ordering" size="30" placeholder="<?php echo $bsc_types->ordering->PlaceHolder ?>" value="<?php echo $bsc_types->ordering->EditValue ?>"<?php echo $bsc_types->ordering->EditAttributes() ?>>
</span><?php echo $bsc_types->ordering->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
<?php if (strval($bsc_types->idProduct->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_idProduct" id="x_idProduct" value="<?php echo ew_HtmlEncode(strval($bsc_types->idProduct->getSessionValue())) ?>">
<?php } ?>
