		...
		foreach ($reviews as $i => $review ) {
			...
			switch ( $theme ) {
				...
				case 'theme-3':
					...
					echo '<li class="'. $hi .' '. $has_rank .'" id="rwp-wbox-'. $review['review_post_id'] .'-'. $review['review_id'] .'">';
						echo $rank_num;
						...
						echo '<a href="'. $review['review_permalink'] .'">';
							...
							echo RWP_Reviewer::get_stars( $review['review_score']['overall'], $review_template,  self::template_field('template_maximum_score', $template, true));

							if( $review['review_type'] == 'UR' ) {
								$count = $review['review_score']['count'];
								$count_label = ( $count == 1 ) ? __(self::template_field('template_users_count_label_s', $template, true), 'reviewer') : __(self::template_field('template_users_count_label_p', $template, true), 'reviewer');
				...
				case 'theme-2':
					...
					echo '<li class="'. $has_rank .'" id="rwp-wbox-'. $review['review_post_id'] .'-'. $review['review_id'] .'">';
						echo $rank_num;
	...
	public static function manage_review( $review, $post_id, $sort ) {
		...
		switch ($sort) {
			...
			case 'top_users_scores':
				...
				$overall 	= array( 'overall' => $data['overall'], 'count' => $data['count'], 'label' => __($template['template_users_score_label'], 'reviewer') );
				break;
			...
			case 'top_score':
				...
				$overall = array( 'overall' => $data, 'count' => 1, 'label' => __($template['template_total_score_label'], 'reviewer') );
