...
class RWP_Rating
{
	...
	public static function ajax_callback()
	{
		...
		switch ( $method ) {
			...
			default: // Slider rating mode
				...
				if( is_array( $_POST['scores'] ) && count( $_POST['scores'] ) == $criteria_count ) {
					$tmin = $template['template_minimum_score'];
					$tmax = $template['template_maximum_score'];
		...
		// Validate privacy
		$privacy = array();
		$gdpr = self::preferences_field('preferences_gdpr', $preferences_option, true);

		if (isset($gdpr['enabled']) && $gdpr['enabled'] && isset($gdpr['terms']) && is_array($gdpr['terms']) && !empty($gdpr['terms']))  {
			foreach ($gdpr['terms'] as $key => $term) {
				if(!isset($_POST['privacy'][$key])) {
					$errors[] = sprintf(__('Bad request, term %s is not set', 'reviewer'), $key);
				} else if(($term['required']) && 'true' != $_POST['privacy'][$key] ) {
					$errors[] = sprintf(__('Privacy term %s must be accepted', 'reviewer'), '<em>"'. $term['name'] .'"</em>');
				} else {
					$privacy[$key] = $_POST['privacy'][$key];
				}
			}
		}
		...
		// Check process res
		if ($process === FALSE) {
			wp_send_json_error( __($template['template_failure_message'], 'reviewer') );
		}
		...
		if( $moderation == 'nothing' ) {
			...
			// Success!
			$res['code'] = 200;
			$res['msg'] = __($template['template_success_message'], 'reviwer');
			...
			// Success!
			$res['code'] = 201;
			$res['msg'] = __(self::template_field('template_moderation_message', $template, true), 'reviewer');
		...
		static::schedule_postmeta_updating($post_id, $review_id);

		wp_send_json_success( $res );

	}

	public static function send_notification( $args = array(), $email = '', $reviewsPageUrl = '' )
	{
		global $wpdb;
	...
	public static function schedule_reviews_notice( $reviews = array(), $email = '') {
		$url = menu_page_url('reviewer-users-ratings-page', false);
		if(! wp_next_scheduled( 'reviewer_notify_about_new_reviews' ) ) {
			wp_schedule_single_event( time() + 10, 'reviewer_notify_about_new_reviews', array( $reviews, $email, $url ) );
	    }
	}
	...
	public static function schedule_postmeta_updating($post, $box) {
		if(! wp_next_scheduled( 'reviewer_update_postmeta_of_box_scores' ) ) {
			wp_schedule_single_event( time() + 5, 'reviewer_update_postmeta_of_box_scores', array( $post, $box ) );
	    }
	}

	public static function unschedule_postmeta_updating() {
		wp_clear_scheduled_hook('reviewer_update_postmeta_of_box_scores');
	}
}
